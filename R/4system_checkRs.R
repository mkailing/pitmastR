...
readr_check <- function(x, sys.colname, date1, min.reads = 2){
  z <- x %>%
    ...
    mutate(op = ifelse(tt.fires>=min.reads, "YES", "NO")) %>%
    rename(reader = 1, date = 2)
  return(z)
}
...
#' 
#' @description
#' Plots the number of times a test tag was fired, indicating whether reader was operating. 
#' 
